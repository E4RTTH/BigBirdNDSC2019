...
def cascaded_train_predict_data(dataset_train, dataset_val, attr_name, classifier, regex, predNum, vectorizercount):
    
    # Remove NaN entries from Benefits attribute
    dataset_train_attr = dataset_train.dropna(subset=['Brand'])
    dataset_train_attr = dataset_train_attr.dropna(subset=['Phone Model'])
    dataset_train_attr = dataset_train_attr.dropna(subset=[attr_name])
    
    # Cleaning the titles
    Xt_train = preprocess_data(dataset_train_attr['title'].values, regex)
    Xt_test = preprocess_data(dataset_val['title'].values, regex)

    y_train = dataset_train_attr[attr_name].values
    
    # Using Count Vectorizer as features
    trainCount = len(Xt_train)
    Xt = Xt_train + Xt_test
    Xt = vectorize_data(CountVectorizer(max_features = vectorizercount), Xt)
    Xt_train = Xt[0:trainCount]
    Xt_test = Xt[trainCount:len(Xt)]
    
    Xb_train = [int(i) for i in dataset_train_attr['Brand'].values]
    Xm_train = [int(i) for i in dataset_train_attr['Phone Model'].values]
    
    Xbm_train = list(zip(Xb_train, Xm_train))
    
    enc = OneHotEncoder()
    enc.fit(Xbm_train)

    Xbm_train = enc.transform(Xbm_train).toarray()
    
    X_train = np.hstack((Xbm_train,Xt_train))
    ...
    Xbm_test = list(zip(y_predb, y_predm))
    Xbm_test = enc.transform(Xbm_test).toarray()
    
    X_test = np.hstack((Xbm_test,Xt_test))
