...
def preprocess_data(titles, regex, attr_name, mobile):
    ...
    for item in titles:
        ...
        if (mobile == True):
            ...
            title = re.sub('tahun|thn', 'year', title)   
            title = re.sub('bulan|bln', 'month', title)
            title = re.sub('(?<=(\d)) (?=(tahun|thn|bulan|bln|year|month|))', '', title)
...
def train_predict_data(dataset_train, dataset_val, attr_name, classifier, regex, predNum, vectorizercount, mobile):
    ...
    # Cleaning the titles
    X_train = preprocess_data(dataset_train_attr['title'].values, regex, attr_name, mobile)
    X_test = preprocess_data(dataset_val['title'].values, regex, attr_name, mobile)
...
classifier200 = RandomForestClassifier(n_estimators = 200, criterion = 'entropy', random_state = 0)
predictionNum = 2
